//
// Created by amartya on 16/11/21.
//
...
void gobkn::receiver(int m) {
    ...
    // else they are correctly received
    ...
    if (f != 5) {
        for (int i = 0; i < fr_send_at_instance; i++) {
            if (rw == arr1[i]) {
                ...
                cout << "RECEIVER : Duplicate frame " << arr1[i] << " is discarded.\n";
        }
        ...
        if (ld == 0)
            cout << "RECEIVER: Frame " << arr1[f1] << " is damaged!\n";
        else
            cout << "(Frame " << arr1[f1] << " is lost.)\n";

        for (i = f1 + 1; i < fr_send_at_instance; i++)
            cout << "RECEIVER: Frame " << arr1[i] << " is discarded.\n";
